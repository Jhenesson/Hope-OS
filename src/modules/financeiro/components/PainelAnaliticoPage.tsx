...
export const PainelAnaliticoPage: React.FC = () => {
    ...
    const chartData = useMemo(() => {
        ...
        const receitaPorMes = filteredData.filteredLancamentos.reduce((acc: Record<string, number>, l: any) => {
            ...
            acc[month] = (acc[month] || 0) + Number(l.valorRecebido || 0);
            return acc;
        ...
        const rawCategoryAggregation = filteredData.filteredLancamentos.reduce((acc: Record<string, number>, l: any) => {
        ...
        // --- 2. Calculate Total ---
        const totalReceitaCategoria: number = (Object.values(rawCategoryAggregation) as number[]).reduce((sum: number, val: number) => sum + val, 0);

        // --- 3. STRICT SORTING (Numeric Descending) ---
        // We create a clean array of objects { name, value } and sort strictly by value
        const sortedNumericData = Object.entries(rawCategoryAggregation)
            .map(([name, value]) => ({ name, value: Number(value) }))
            .sort((a, b) => b.value - a.value); // Sort largest to smallest

        // --- 4. Map to Charts ---
        
        // Pie Chart Data: Uses the SORTED array. We append the % string to the name here.
        // Since sortedNumericData is already sorted, distribuicaoCategoria will preserve that order.
        const distribuicaoCategoria = sortedNumericData.map(item => {
            const percentage = totalReceitaCategoria > 0 ? (item.value / totalReceitaCategoria) * 100 : 0;
            return { 
                name: `${item.name} (${percentage.toFixed(0)}%)`, // Label in legend
                value: item.value
            };
        });

        // Bar Chart Data: Uses the SAME sorted array, but keeps clean names for axis
        const topCategoriasValor = sortedNumericData.map(item => ({ 
            name: item.name,
            value: item.value
        }));
